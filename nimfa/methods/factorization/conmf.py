...
"""
###################################
CoNmf (``methods.factorization.conmf``)
###################################

**Contrastive Nonnegative Matrix Factorization (NMF)** [Potluru2008]_.
...
class CoNmf(nmf_std.Nmf_std):
    ...
    :param group_ind: The indices to specify groups to contrast. 
    :type group_ind: Instance of the :class:`numpy.ndarray`.

    :param lambd: Weight for contrastive term.
       Default is 0.
    ...
    **Stopping criterion**
    ...
    :type test_conv: `int`
    """
    def __init__(self, V, group_ind, lambd=0., seed=None, W=None, H=None, rank=30, max_iter=30,
        ...
        self._conn_change = 0
        assert self.group_ind.shape[0] == V.shape[0]
    ...
    def euclidean(self):
        ...
        V1 = self.V[self.group_ind==0, :]
        V2 = self.V[self.group_ind==1, :]
        W1 = self.W[self.group_ind==0, :]
        W2 = self.W[self.group_ind==1, :]
        d = np.mean(W1, axis=0) - np.mean(W2, axis=0)
        self.H = multiply(
            self.H, elop(dot(self.W.T, self.V), dot(self.W.T, dot(self.W, self.H)), div))
        W1 = multiply(
            W1, elop(
                dot(V1, self.H.T) + (self.lambd * np.abs(multiply(d, d<0))),
                dot(W1, dot(self.H, self.H.T)) + (self.lambd * np.abs(multiply(d, d>0))),
                div))
        W2 = multiply(
            W2, elop(
                dot(V2, self.H.T) + (self.lambd * np.abs(multiply(d, d>0))),
                dot(W2, dot(self.H, self.H.T)) + (self.lambd * np.abs(multiply(d, d<0))),
                div))
        if sp.isspmatrix(self.W):
            self.W = sp.vstack([W1, W2])
        else:
            self.W = np.concatenate([W1, W2], axis=0)
    ...
    def fro(self):
        ...
        W2 = self.W[self.group_ind==1, :]
        d = np.mean(W1, axis=0) - np.mean(W2, axis=0)
        rec_err_1 = norm(V1 - dot(W1, self.H), p="fro")
        rec_err_2 = norm(V2 - dot(W2, self.H), p="fro")
        return (0.5 * rec_err_1) + (0.5 * rec_err_2) - norm(self.lambd * d, p=1)
